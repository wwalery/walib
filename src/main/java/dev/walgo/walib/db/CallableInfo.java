  ...
  private List<ParameterInfo> getProcedureColumns(String namePattern) throws SQLException {
    ...
    try (ResultSet rs = meta.getProcedureColumns(catalog, locSchema, locName, null)) {
      ...
      while (rs.next()) {
        ...
        column.columnType(ColumnType.fromProcedure(rs.getShort(AbstractFieldInfo.COLUMN_COLUMN_TYPE))); // column type
        column.type(rs.getInt(AbstractFieldInfo.COLUMN_DATA_TYPE)); // SQL type from java.sql.Types
        column.typeName(rs.getString(AbstractFieldInfo.COLUMN_TYPE_NAME)); // Data source dependent type name, for a UDT
                                                                           // the type name
        // is fully qualified
        column.size(rs.getInt(AbstractFieldInfo.COLUMN_PRECISION)); // column size. For char or date types this is the
                                                                    // maximum number
        // of characters, for numeric or decimal types this is precision.
        column.digits(rs.getShort(AbstractFieldInfo.COLUMN_SCALE)); // scale - null is returned for data types where
                                                                    // SCALE is not
        // applicable.
        column.radix(rs.getShort(AbstractFieldInfo.COLUMN_RADIX)); // Radix (typically either 10 or 2)
        column.isNullable(rs.getInt(AbstractFieldInfo.COLUMN_NULLABLE) == DatabaseMetaData.columnNullable); // is NULL
                                                                                                            // allowed.
        try {
          column.position(rs.getInt(AbstractFieldInfo.COLUMN_ORDINAL_POSITION)); // he ordinal position, starting from
                                                                                 // 1,
  ...
  private List<ParameterInfo> getFunctionColumns(String namePattern) throws SQLException {
    ...
    try (ResultSet rs = meta.getFunctionColumns(catalog, locSchema, locName, null)) {
      ...
      while (rs.next()) {
        ...
        column.name(rs.getString(AbstractFieldInfo.COLUMN_COLUMN_NAME)); // column name
        column.columnType(ColumnType.fromFunction(rs.getShort(AbstractFieldInfo.COLUMN_COLUMN_TYPE))); // column type
        column.type(rs.getInt(AbstractFieldInfo.COLUMN_DATA_TYPE)); // SQL type from java.sql.Types
        column.typeName(rs.getString(AbstractFieldInfo.COLUMN_TYPE_NAME)); // Data source dependent type name, for a UDT
                                                                           // the type name
        // is fully qualified
        ...
        column.position(rs.getInt(AbstractFieldInfo.COLUMN_ORDINAL_POSITION)); // the ordinal position, starting from 1,
