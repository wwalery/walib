    ...
    if (tables == null) {
      ...
      try (ResultSet rs = meta.getTables(catalog, locSchema, locName, null)) {
        ...
        while (rs.next()) {
          ...
          table.remarks = rs.getString(AbstractFieldInfo.COLUMN_REMARKS); // comment describing column (may be null)
    ...
    if (functions == null) {
      ...
      try (ResultSet rs = meta.getFunctions(catalog, locSchema, locName)) {
        ...
        while (rs.next()) {
          ...
          func.comment = rs.getString(AbstractFieldInfo.COLUMN_REMARKS); // explanatory comment on the procedure
    ...
    if (procedures != null) {
      ...
      try (ResultSet rs = meta.getProcedures(catalog, locSchema, locName)) {
        ...
        while (rs.next()) {
          ...
          proc.comment = rs.getString(AbstractFieldInfo.COLUMN_REMARKS); // explanatory comment on the procedure
