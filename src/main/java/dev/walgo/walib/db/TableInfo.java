    ...
    if (fields == null) {
      ...
      try (ResultSet rs = meta.getColumns(catalog, locSchema, locTable, null)) {
        ...
        while (rs.next()) {
          ...
          field.typeName(rs.getString(AbstractFieldInfo.COLUMN_TYPE_NAME)); // Data source dependent type name, for a
                                                                            // UDT the type name
          // is fully qualified
          field.size(rs.getInt("COLUMN_SIZE")); // column size. For char or date types this is the maximum number
          ...
          field.isNullable(rs.getInt(AbstractFieldInfo.COLUMN_NULLABLE) == DatabaseMetaData.columnNullable); // is NULL
                                                                                                             // allowed.
          field.comment(rs.getString(AbstractFieldInfo.COLUMN_REMARKS)); // comment describing column (may be null)
          field.defaultValue(rs.getString("COLUMN_DEF")); // default value (may be null)
          field.position(rs.getInt(AbstractFieldInfo.COLUMN_ORDINAL_POSITION)); // index of column in table (starting at
                                                                                // 1)
